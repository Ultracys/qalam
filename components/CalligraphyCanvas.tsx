...
import {
  drawPracticeGuides,
  drawTrainingTemplate,
} from '@/components/PracticeGuides';
...
export function CalligraphyCanvas() {
  ...
  const render = useCallback(() => {
    ...
    if (state.mode === 'training' || state.mode === 'custom-training') {
      const fontStyle =
        state.practice === 'ruqah'
          ? 'ruqah'
          : state.practice === 'diwani'
            ? 'diwani'
            : state.practice === 'kufi'
              ? 'kufi'
              : 'naskh';
      const fontFamily = getComputedStyle(document.body)
        .getPropertyValue(`--font-training-${fontStyle}`)
        .trim();
      drawTrainingTemplate(
        ctx,
        rect.width,
    ...
    if (traceImage.current) {
      ...
      const scale = Math.min(
        ...
        (rect.height * 0.7) / image.naturalHeight,
        1,
      );
      ctx.save();
  ...
  const eraseAt = (point: { x: number; y: number }) => {
    ...
    const match = [...canvasStore.getState().strokes]
      .reverse()
      .find((stroke) =>
        stroke.points.some(
          (sample, index) =>
            index > 0 &&
            pointSegmentDistance(point, stroke.points[index - 1], sample) <
              radius,
        ),
      );
  ...
  const handlePointerDown = (event: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    activeStroke.current = {
      ...
      nibWidth: state.brush.nibWidth,
      nibAngle: state.brush.nibAngle,
      pressureEnabled: state.brush.pressureEnabled,
      pressureSensitivity: state.brush.pressureSensitivity,
  ...
  const handlePointerMove = (event: React.PointerEvent<HTMLCanvasElement>) => {
    if (event.pointerType === 'touch') {
      if (activePen.current !== null || !touches.current.has(event.pointerId))
        return;
          ...
          const nextZoom = Math.max(
            ...
            Math.min(
              ...
              transform.current.zoom *
                (pinchDistance / lastPinchDistance.current),
            ),
          );
          transform.current.zoom = nextZoom;
          setZoom(nextZoom);
        }
    ...
    const point = canvasPoint(event);
    if (mousePan.current?.id === event.pointerId) {
      transform.current.x += event.clientX - mousePan.current.x;
      transform.current.y += event.clientY - mousePan.current.y;
      mousePan.current = {
        id: event.pointerId,
        x: event.clientX,
        y: event.clientY,
    ...
    if (state.tool === 'eraser' && event.buttons) {
      eraseAt(point);
      return;
    }
    ...
    for (const sampleEvent of events) {
      ...
      const view = transform.current;
      const sample = {
        x: (sampleEvent.clientX - rect.left - view.x) / view.zoom,
        y: (sampleEvent.clientY - rect.top - view.y) / view.zoom,
      };
      const previous = stroke.points.at(-1)!;
      let current = pointFromPointer(sampleEvent, sample.x, sample.y, previous);
      const stabilization = { off: 0, low: 12, medium: 28, high: 45 }[
        state.brush.stabilizer
      ];
      current = smoothPoint(
        previous,
        current,
        Math.max(state.brush.smoothing, stabilization),
      );
      stroke.points.push(
        ...interpolatePoints(previous, current, stroke.nibWidth * 0.28),
      );
      setDebugData({
  ...
  const finishPointer = (event: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (activeStroke.current?.points.length)
      canvasStore.addStroke(activeStroke.current);
  ...
  const changeZoom = (delta: number) => {
    const next = Math.max(0.45, Math.min(4, transform.current.zoom + delta));
    transform.current.zoom = next;
    setZoom(next);
    queueRender();
  };
  ...
  const exportPng = (transparent: boolean) => {
    ...
    if (!transparent) {
      ...
      anchor.download = 'qalam-canvas.png';
      anchor.click();
      return;
    }
    ...
    anchor.download = 'qalam-canvas-transparent.png';
    anchor.click();
  };
  const exportSvg = () => {
  ...
  const saveProject = () => {
    const project = {
      ...
      state: { ...canvasStore.getState(), redoStack: [] },
    };
    downloadText(
      JSON.stringify(project),
      `qalam-${new Date().toISOString().slice(0, 10)}.qalam`,
      'application/json',
    );
  };
  const openProject = async (file?: File) => {
    ...
    try {
      ...
      if (
        project?.format !== 'qalam-canvas' ||
        project?.version !== 1 ||
        !Array.isArray(project?.state?.strokes)
      )
        throw new Error('invalid project');
      canvasStore.loadProject(project.state);
      resetView();
    } catch {
      alert('تعذر فتح الملف. اختر ملف مشروع محفوظ من لوحة قلم.');
    }
  };
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'z') {
        event.preventDefault();
        event.shiftKey ? canvasStore.redo() : canvasStore.undo();
        return;
      }
    ...
    <main className="app-shell" dir="rtl">
      ...
      <section className="workspace">
        ...
        <div className="canvas-stage">
          <canvas
            ref={canvasRef}
            aria-label="لوحة الكتابة بالخط العربي"
            ...
            {!traceUrl ? (
              <label className="trace-upload">
                ...
                <input
                  type="file"
                  accept="image/*"
                  onChange={(event) => uploadTrace(event.target.files?.[0])}
                />
              </label>
            ) : (
              <>
                ...
                />
                <Plus />
                <span>{traceOpacity}%</span>
              </>
            )}
          </div>
          {state.debug && (
            <output className="debug-panel">
              <b>بيانات القلم</b>
              <span>
                Pressure <strong>{debugData.pressure.toFixed(2)}</strong>
              </span>
              <span>
                Tilt X <strong>{debugData.tiltX}°</strong>
              </span>
              <span>
                Tilt Y <strong>{debugData.tiltY}°</strong>
              </span>
              <span>
                Velocity <strong>{debugData.velocity.toFixed(2)}</strong>
              </span>
              <span>
                Pointer <strong>{debugData.pointerType}</strong>
              </span>
              <span>
                Nib <strong>{state.brush.nibAngle}°</strong>
              </span>
            </output>
          )}
        </div>
